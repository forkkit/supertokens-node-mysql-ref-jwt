 ...
 */
export async function createNewSession(userId: string, jwtPayload?: any, sessionData?: any): Promise<{
    session: {
        handle: string,
        userId: string,
        jwtPayload: any,
 ...
 */
export async function getSession(idRefreshToken: string, accessToken: string): Promise<{
    session: {
        handle: string,
        userId: string,
        jwtPayload: any
 ...
 */
export async function refreshSession(idRefreshToken: string, refreshToken: string): Promise<{
 ...
 * @throws AuthError, GENERAL_ERROR, UNAUTHORISED
 */
async function refreshSessionHelper(refreshToken: string, refreshTokenInfo: {
    ...
    try {
        ...
        if (sessionInfo.refreshTokenHash2 === hash(hash(refreshToken))) {
            ...
            return {
                ...
                newIdRefreshToken: { value: generateUUID(), expires: newRefreshToken.expiry }
            };
        }
