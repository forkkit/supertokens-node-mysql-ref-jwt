...
const validateAndNormalise = config => {
    ...
    const connectionLimit = utils_1.sanitizeNumberInput(
    ...
    if (tablesMysqlInputConfig !== undefined) {
        const signingKey = utils_1.sanitizeStringInput(
            tablesMysqlInputConfig.signingKey
        );
        const refreshTokens = utils_1.sanitizeStringInput(
    ...
    if (accessTokenInputConfig !== undefined) {
        ...
        if (signingKeyInputConfig !== undefined) {
            const dynamic = utils_1.sanitizeBooleanInput(
                signingKeyInputConfig.dynamic
            );
            let updateInterval = utils_1.sanitizeNumberInput(
        ...
        let validity = utils_1.sanitizeNumberInput(
            accessTokenInputConfig.validity
        );
    ...
    let validity = utils_1.sanitizeNumberInput(
        refreshTokenInputConfig.validity
    );
    if (validity !== undefined) {
    ...
    const renewTokenPath = utils_1.sanitizeStringInput(
