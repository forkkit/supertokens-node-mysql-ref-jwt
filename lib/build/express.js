...
function createNewSession(res, userId, jwtPayload, sessionData) {
    return __awaiter(this, void 0, void 0, function*() {
        ...
        utils_1.setAntiCsrfTokenInResponseHeaders(res, response.antiCsrfToken);
...
function getSession(req, res, enableCsrfProtection) {
    return __awaiter(this, void 0, void 0, function*() {
        ...
        try {
            ...
            let antiCsrfToken = enableCsrfProtection ? utils_1.getAntiCsrfTokenFromRequestHeaders(req) : undefined;
            if (enableCsrfProtection && antiCsrfToken === undefined) {
                throw Error("anit-csrf token not found in headers");
            }
            let response = yield SessionFunctions.getSession(accessToken, antiCsrfToken);
...
function refreshSession(req, res) {
    return __awaiter(this, void 0, void 0, function*() {
        ...
        try {
            ...
            cookie_1.attachIdRefreshTokenToCookie(
                res,
                response.newIdRefreshToken.value,
                response.newIdRefreshToken.expires
            );
            utils_1.setAntiCsrfTokenInResponseHeaders(res, response.newAntiCsrfToken);
            return new Session(response.session.handle, response.session.userId, response.session.jwtPayload, res);
        } catch (err) {
            if (
