...
function createTablesIfNotExists(
    ...
    return __awaiter(this, void 0, void 0, function*() {
        ...
        `;
        const signKeyTableQueryPromise = connection.executeQuery(
            signKeyTableQuery,
            []
        );
        const refreshTokensTableQueryPromise = connection.executeQuery(
...
function getKeyValueFromKeyName_Transaction(connection, keyName) {
    return __awaiter(this, void 0, void 0, function*() {
        ...
        connection.throwIfTransactionIsNotStarted(
            "expected to be in transaction when reading signing keys"
        );
...
function insertKeyValueForKeyName_Transaction(
    connection,
    keyName,
    keyValue,
    createdAtTime
...
function updateSessionData(connection, sessionHandleHash1, sessionData) {
    return __awaiter(this, void 0, void 0, function*() {
        ...
        } SET session_info = ? WHERE session_handle_hash_1 = ?`;
...
function getSessionData(connection, sessionHandleHash1) {
    return __awaiter(this, void 0, void 0, function*() {
        const config = config_1.default.get();
        let query = `SELECT session_info FROM ${
            config.mysql.tables.refreshTokens
...
function getSessionInfo_Transaction(connection, sessionHandleHash1) {
    return __awaiter(this, void 0, void 0, function*() {
        const config = config_1.default.get();
        connection.throwIfTransactionIsNotStarted(
            "expected to be in transaction when reading session data"
        );
        let query = `SELECT user_id,
    refresh_token_hash_2, session_info,
    expires_at, jwt_user_payload FROM ${
        config.mysql.tables.refreshTokens
    } WHERE session_handle_hash_1 = ? FOR UPDATE`;
        let result = yield connection.executeQuery(query, [sessionHandleHash1]);
        if (result.length === 0) {
            return undefined;
        }
...
function updateSessionInfo_Transaction(
    connection,
    sessionHandleHash1,
    refreshTokenHash2,
    sessionData,
    ...
    return __awaiter(this, void 0, void 0, function*() {
        const config = config_1.default.get();
        connection.throwIfTransactionIsNotStarted(
            "expected to be in transaction when updating session data"
        );
        let query = `UPDATE ${
            config.mysql.tables.refreshTokens
        } SET refresh_token_hash_2 = ?, 
