---
id: version-4.3.X-manipulating-session-data
...
async function changeSessionInfoWithSessionObject(sessionHandle) {
    try {
        let sessionInfo = await SuperTokens.getSessionInfo(sessionHandle);
        await SuperTokens.updateSessionInfo(sessionHandle, {comment: "new session info"});
    } catch (err) {
        if (SuperTokens.Error.isErrorFromAuth(err)) {
            if (err.errType === SuperTokens.Error.GENERAL_ERROR) {
